...
//   async function handlePromptSubmit(userPrompt: string) {
//     if (!userPrompt.trim() || isLoading) return;
...
//       notifySidebar();
//       chat = newChat;
//       messagesHistory = newChat.messages;
//     } else {
//       chat = getChat(currentChatId);
//       if (!chat) {
//         console.error("No chat found with ID", currentChatId);
//         setIsLoading(false);
//         return;
//       }
//       messagesHistory = [...chat.messages, userMessage];
//       setMessages(messagesHistory);
//       saveChat({ ...chat, messages: messagesHistory });
//     }
...
//       saveChat({ ...chat, messages: finalMessages });
//     } catch (error) {
//       console.error(error);
...
//   ) {
//     if (!editedPrompt.trim() || isLoading || !currentChatId) return;
...
//     saveChat({ ...chat, messages: messagesHistory, title: newTitle });
//     notifySidebar();
...
//       setMessages(finalMessages);
//       saveChat({ ...chat, messages: finalMessages, title: newTitle });
//     } catch (error) {
//       console.error(error);
//       const errorMessage: Message = {
//         role: "assistant",
...
import { Prism as SyntaxHighlighter } from "react-syntax-highlighter";
import { oneDark } from "react-syntax-highlighter/dist/esm/styles/prism";
import { Bot, User, Pencil, Copy, Check, RefreshCw } from "lucide-react";
import AI_Prompt from "@/components/kokonutui/ai-prompt";
...
function NewChat() {
  ...
  function handleEditSubmit(index: number, newText: string) {
    ...
    setEditingMessageIndex(null);
    setEditingMessageText("");
    handleResubmit(newText, newMessages); // Call context function
  }
  ...
  return (
    <div className={cn("flex flex-col h-screen", "relative overflow-hidden")}>
      {!hasConversationStarted ? (
        ...
        <div className="flex-1 overflow-y-auto" ref={messagesContainerRef}>
          <div className="w-full max-w-4xl mx-auto px-3 py-6 sm:py-8">
                ...
                >
                  {msg.role === "assistant" && (
                    <>
                      ...
                      >
                        <span className="text-xs text-neutral-500 dark:text-neutral-400 absolute -top-5 left-0">
                        ...
                        {msg.isError && (
                          <button
                            ...
                            className="flex items-center gap-2 mt-2 px-3 py-1 rounded-md text-sm font-medium text-red-600 bg-red-100 hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900"
                          >
                  ...
                  {msg.role === "user" && (
                    <>
                      ...
                      {editingMessageIndex === i ? (
                        ...
                        >
                          {/* --- ADDED TIMESTAMP --- */}
                          <span className="text-xs text-neutral-500 dark:text-neutral-400 absolute -top-5 right-12">
                            {formatTimestamp(msg.timestamp)}
                          </span>
                          {/* --- END TIMESTAMP --- */}
                          <p className="whitespace-pre-wrap">{msg.text}</p>
